...
if [ $number_of_tables -lt 3 ]; then
  php bin/console doctrine:schema:update --force # This isn't recommended for production but for the demo
fi

# For saving time with the testing, the migration is done using a flag in this file.
if [ $MIGRATION = "ON" ]; then
  # Mocking a migration to force symfony to do changes in the database
